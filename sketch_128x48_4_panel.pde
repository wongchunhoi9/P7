...
void draw() { 
 ...
 // shuffleArray(cells);
  ...
  //LEFT SCREEN DISPLAY    
  DrawPixelArrayInOrderLeftScreen(cells,dotDisplayintervalLeftScreen,LeftScreenPositionX,LeftScreenPositionY,60);   //dotDisplayinterval1
  
  //RIGHT SCREEN DISPLAY 
  DrawPixelArrayInOrderRightScreen(cells2,dotDisplayintervalRightScreen,RightScreenPositionX,RightScreenPositionY,300
  );   //dotDisplayinterval1
...
void drawLine() { 
  ...
  if (millis() - timer3 >= 3000) { //3000 for 3 secounds
    drawlineCounter ++; 
    stroke(255);
    line(pointAx+MidScreenPositionX,pointAy+MidScreenPositionY,pointBx+MidScreenPositionX+32,pointBy+MidScreenPositionY);
  
     timer3 = millis();
  }
  if (drawlineCounter>10) {  //clear afyter 10 lines
    fill(0);
    noStroke();
    rect(MidScreenPositionX,MidScreenPositionY,panelSizeX*2,panelSizeY);
    drawlineCounter=0;
